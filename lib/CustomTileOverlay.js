...
CustomTileOverlay = function (map, opacity, tileUrl) {
...
//			return this.tiles[n];
	...
	google.maps.event.addListener(map, 'tilesloaded', function () {
        self.deleteHiddenTiles(map.getZoom());
    });
...
CustomTileOverlay.prototype.remove = function () {
	var tilesLength = this.tiles.length;
	for (var i = 0; i < tilesLength; i++) {
		delete this.tiles[i];
	}
...
CustomTileOverlay.prototype.getTileUrl = function (coord, zoom) {
	...
	return this.localurl + "/" + zoom + "/" + coord.x + "/" + coord.y + ".png";
};
